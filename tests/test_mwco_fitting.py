...
class TestMWCOFitting(unittest.TestCase):
    def test_fit_curve(self):
        # Add more data points to match the number of parameters
        mw = [100, 200, 300, 400]
        ...
        self.assertTrue(np.all(fitted_curve < 100))  # Basic check
        self.assertAlmostEqual(fitted_curve[0], 40, delta=1)  # Adjust delta
